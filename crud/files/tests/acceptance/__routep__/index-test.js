...
import Page from "tchw/tests/pages/<%= pRouteFiles %>/index";
import presignHandler from "tchw/mirage/helpers/presign-handler";

const PAGE_URL = <%= pCaps %>_URL;

module("Acceptance | <%= pRouteFiles %>/index", function (hooks) {

  setupApplicationTest(hooks);
  setupMirage(hooks);
  setupCustomAssertions(hooks);

  test("Auth: visiting /<%= pRouteFiles %>/index unauthenticated", async function (assert) {
  ...
  test("Auth: visiting /<%= pRouteFiles %>/index", async function (assert) {
  ...
  test("Pagination: loading page 1 of /<%= pRouteFiles %>/index", async function (assert) {

    this.server.createList("<%= s %>", 42);
  ...
  test("Pagination: loading page 2 with custom page size of /<%= pRouteFiles %>/index", async function (assert) {
    this.server.createList("<%= s %>", 42);
  ...
  test("Pagination: loading a page outside of limit of /<%= pRouteFiles %>/index", async function (assert) {
    this.server.createList("<%= s %>", 42);
  ...
  test("No Results: /<%= pRouteFiles %>/index", async function (assert) {
    await authenticateSession(AUTHENTICATED_ADMIN);
  ...
  test("Sorting: /<%= pRouteFiles %>/index", async function (assert) {
  ...
  test("Searching /<%= pRouteFiles %>/index", async function (assert) {

    // NOTE:
    // Update these seeds with your own to test the search functionality
    this.server.createList("<%= s %>", 8, {
      firstName: "bar",
      lastName: "bar",
      email: "bar@mail.com",
    });
    this.server.createList("<%= s %>", 2, {
      firstName: "foo",
    });
  ...
  test("Filter: set filter /<%= pRouteFiles %>/index", async function (assert) {
  ...
  test("Filter: load filter from url /<%= pRouteFiles %>/index", async function (assert) {
  ...
  test("Exporting: /<%= pRouteFiles %>/index", async function (assert) {
  ...
  test("Navigation: /<%= pRouteFiles %>/index", async function (assert) {
    const resources = this.server.createList("<%= s %>", 3);
    ...
    assert.equal(currentURL(), `${<%= sCaps %>_INDEX_URL.replace(':id', id)}`, "url is correct");

    assert.equal(
      Page.title,
      `View <%= sUpper %>: ${resource.id}`,
