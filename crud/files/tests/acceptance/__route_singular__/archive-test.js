...
import Page from "<%= appName %>/tests/pages/internal/<%= dasherizedSingular %>/archive";

const PAGE_URL = <%= capitalizedSingular %>_ARCHIVE_URL;

module("Acceptance | <%= routePathSingular %>/archive", function (hooks) {
  setupApplicationTest(hooks);
  setupMirage(hooks);
  setupCustomAssertions(hooks);

  test("Auth: visiting /<%= routePathSingular %>/archive unauthenticated", async function (assert) {
  ...
  test("Auth: visiting /<%= routePathSingular %>/archive", async function (assert) {
    const resource = this.server.create("<%= dasherizedSingular %>"),
  ...
  test("Navigation: back to list view /<%= routePathSingular %>/archive", async function (assert) {
    const resource = this.server.create("<%= dasherizedSingular %>")
  ...
  test("Navigation: load read view /<%= routePathSingular %>/archive", async function (assert) {
    const resource = this.server.create("<%= dasherizedSingular %>"),
      describe = `<%= titleSingular %> ${resource.id}`;
    ...
    assert.equal(currentURL(), <%= capitalizedSingular %>_INDEX_URL.replace(':id', resource.id), "url is correct");
    ...
    assert.equal(Page.title, `View <%= titleSingular %>: ${describe}`, "page heading is correct");
  });

  test("Navigation: load edit view /<%= routePathSingular %>/archive", async function (assert) {
  ...
  test("Archive: <%= titleSingular %> /<%= routePathSingular %>/archive", async function (assert) {
    const resource = this.server.create("<%= dasherizedSingular %>")
